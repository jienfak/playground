...
if test $systemd_version -gt 219
    #complete -f -c systemctl -l firmware-setup -n "__fish_seen_subcommand_from reboot" -d "Reboot to EFI setup" complete -f -c systemctl -l now -n "__fish_seen_subcommand_from enable" -d "Also start unit"
    complete -f -c systemctl -l now -n "__fish_seen_subcommand_from disable mask" -d "Also stop unit"
end

#for i in (string match(systemctl list-unit-files | cat))
#	set description (string match -r '(\*)\s+(\w+).(\w+)\s+-(( \w+)*)' (systemctl status $i))
#	complete -x -c systemctl -d $description -a (echo $i| cut -d : -f 1)
#end
#$string match -r '(\w+\.\w+)\s+(\w+)' (systemctl list-unit-files | cat)
